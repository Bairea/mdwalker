...
func TestScanIncludesMarkdownOnly(t *testing.T) {
	root := t.TempDir()
	...
	if !got["notes.md"] {
		t.Fatalf("Scan() missing markdown file notes.md; got paths %#v", got)
	}
	for _, unwanted := range []string{"diagram.png", "photo.jpg", "ignored.txt"} {
		if got[unwanted] {
			t.Fatalf("Scan() included non-markdown file %q", unwanted)
		}
	}
}
